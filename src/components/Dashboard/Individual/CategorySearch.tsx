...
import { fetchCategories, fetchQuestionSets } from '../../../services/api/CategorySearchService';
...
const CategorySearch: React.FC = () => {
 ...
 console.log("matching set recieved on categorySearch",matchingQuestionSets)
  ...
  const handleItemClick = async (item: string, level: number) => {
    ...
    const selectedCategory = findCategory(categoriesData, selectedItems.slice(0, level + 1));
    console.log('Selected category:', selectedCategory);
  
    const itemHasChildren = selectedCategory && selectedCategory.subcategories && Object.keys(selectedCategory.subcategories).length > 0;
  ...
  const findCategory = (categories: Record<string, Category>, path: string[]): Category | null => {
  ...
  const filterCategories = (categories: Record<string, Category>, term: string): string[] => {
  ...
  const renderSubcategories = (subcategories: Record<string, Category>, level: number) => {
    ...
    return Object.keys(subcategories).map((subcategory, index) => (
      <div key={index} className="relative group">
        <a
          href="#"
          className={`block px-4 py-2 text-sm ${selectedItems[level] === subcategory ? 'bg-blue-500 text-white' : 'text-gray-700 hover:bg-gray-100'}`}
          onClick={() => handleItemClick(subcategory, level)}
        >
          {subcategory}
        </a>
        {subcategories[subcategory].subcategories && Object.keys(subcategories[subcategory].subcategories!).length > 0 && selectedItems[level] === subcategory && (
          <div className="absolute left-full top-0 hidden mt-1 group-hover:block">
            <div className="w-48 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5">
              {renderSubcategories(subcategories[subcategory].subcategories!, level + 1)}
            </div>
          </div>
        )}
      </div>
  ...
  const renderCategories = (categories: Record<string, Category>, level: number = 0) => {
    return Object.keys(categories).map((mainCategory) => {
      ...
      return (
        <div key={mainCategory} className="relative group">
          <button
            className={`inline-flex items-center px-3 py-2 text-sm font-medium ${selectedItems[level] === mainCategory ? 'text-blue-500' : 'text-gray-700 hover:text-gray-900'}`}
  ...
  return (
    <div className="container mx-auto px-4" ref={menuRef}>
      ...
      <nav className={`flex flex-wrap lg:flex-nowrap font-spline space-x-4 ${isMenuOpen ? '' : 'hidden lg:flex'}`}>
