        ...
        <div className="flex flex-col grow shrink-0 my-auto basis-0 w-fit max-md:max-w-full">
          {loading ? (
            ...
            <>
              ...
              <div onClick={toggleViewSample} className="flex w-full gap-3 justify-end items-center text-gray-600 px-4 py-2 cursor-pointer">
  <span className="text-sm">View Sample</span> <span><MdArrowOutward  size={15}/></span>
</div>
...

              {
                viewSample &&(
                  ...
                  <img src={noRecordFound} alt="noRecordFound" className="sm:w-2/5 mx-auto " />
                  </div>
                )
              }
             {
                    ...
                    <div className="flex items-center gap-5 max-md:flex-wrap max-md:max-w-full">
                      <div className="flex flex-col flex-auto">
                        <div className="flex justify-between">
                          ...
                          <div className={`text-sm font-spline ${getLabelColor(result.rating)}`}>
                            {getLabel(result.rating)}
                          </div>
                        </div>
                        <div className="flex flex-col justify-center mt-4 rounded-md bg-zinc-300 w-full">
                          <div
                            className={`shrink-0 ${getBarColor(result.rating)} rounded-md h-[5px]`}
                            style={{ width: `${((result.rating + 0.2) / 5) * 100}%` }}
                          />
                        </div>
                      </div>
                      <div className="relative flex items-center justify-center">
                        <svg className="w-16 h-16">
                          ...
                          />
                          <circle
                            className={`transition duration-300 ${getCircleColor(result.rating)}`}
                            strokeWidth="2"
                            strokeLinecap="round"
                            stroke="currentColor"
                            fill="transparent"
                          ...
                          <text
                            ...
                            dy=".3em"
                            textAnchor="middle"
                            className={`font-spline text-xs ${getColor(result.rating)}`}
                            fill="currentColor"
                          >
            ...
            results.map((result) => (
              <div key={result.id} className="mb-10 max-md:mb-10">
                <div className="flex items-center gap-5 max-md:flex-wrap max-md:max-w-full">
                  <div className="flex flex-col flex-auto">
                    <div className="flex justify-between">
                      <div className="text-base leading-4 font-spline text-slate-800">
